...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const res = await authFetch("https://uems-rz8o.onrender.com/api/student/notifications/results");
        ...
        const checkAttendance = async () => {
            ...
            try {
                const res = await authFetch(`https://uems-rz8o.onrender.com/api/attendance/student/${user.studentId}`);
                if (!res.ok) return;
                const data = await res.json();
                if (data.length > 0) {
                        ...
                        <div className="bg-slate-800 border-2 border-emerald-500/50 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl transform scale-100 transition-all relative">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    try {
                                        const res = await authFetch(`https://uems-rz8o.onrender.com/api/student/notifications/${previousNotification.id}/seen`, { method: "PUT" });
